...
if ( ! current_user_can( 'edit_posts' ) ) {
	wp_die(
		'<h1>Cheatin&#8217; uh?</h1>' .
		'<p>You are not allowed to edit comments.</p>',
		403
	);
}
...
if ( $post_id ) {
	...
	if ( $comments_count->moderated > 0 ) {
		$title = sprintf( 'Comments (%1$s) on &#8220;%2$s&#8221;',
			number_format_i18n( $comments_count->moderated ),
			$draft_or_post_title
		);
	} else {
		$title = sprintf( 'Comments on &#8220;%s&#8221;',
			$draft_or_post_title
		);
	}
} else {
...
if ( isset( $_REQUEST['error'] ) ) {
	...
	switch ( $error ) {
		case 1 :
			...
			break;
		case 2 :
			$error_msg = 'You are not allowed to edit comments on this post.';
			break;
	}
	if ( $error_msg )
