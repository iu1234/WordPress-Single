...
class WP_Media_List_Table extends WP_List_Table {
	...
	protected function get_views() {
		...
		$type_links['detached'] = '<option value="detached"' . ( $this->detached ? ' selected="selected"' : '' ) . '>Unattached</option>';
...
?>
	...
	public function get_columns() {
		...
		if ( !$this->detached ) {
			$posts_columns['parent'] = 'Uploaded to';
	...
	public function column_cb( $post ) {
		if ( current_user_can( 'edit_post', $post->ID ) ) { ?>
			<label class="screen-reader-text" for="cb-select-<?php echo $post->ID; ?>"><?php
				echo sprintf( 'Select %s', _draft_or_post_title() );
			?></label>
	...
	public function column_date( $post ) {
		if ( '0000-00-00 00:00:00' === $post->post_date ) {
			...
			if ( ( abs( $t_diff = time() - $time ) ) < DAY_IN_SECONDS ) {
				if ( $t_diff < 0 ) {
					$h_time = sprintf( '%s from now', human_time_diff( $time ) );
				} else {
...
?>
			...
			if ( $user_can_edit ):
				...
				printf(
					...
					$detach_url,
					esc_attr( sprintf( 'Detach from &#8220;%s&#8221;', $title ) ),
					'Detach'
				);
			endif;
		} else { ?>(Unattached)<br />
			<?php if ( $user_can_edit ) {
				...
				printf(
					...
					$post->ID,
					esc_attr( sprintf( 'Attach &#8220;%s&#8221; to existing content', $title ) ),
					'Attach'
				);
			}
		}
		...
		if ( $this->detached ) {
			if ( current_user_can( 'edit_post', $post->ID ) ) {
				$actions['edit'] = sprintf(
					...
					get_edit_post_link( $post->ID ),
					esc_attr( sprintf( 'Edit &#8220;%s&#8221;', $att_title ) ),
					'Edit'
				);
			}
			if ( current_user_can( 'delete_post', $post->ID ) ) {
				if ( EMPTY_TRASH_DAYS && MEDIA_TRASH ) {
					$actions['trash'] = sprintf(
						...
						esc_attr( sprintf( 'Move &#8220;%s&#8221; to the Trash', $att_title ) ),
						'Trash'
					);
				} else {
					...
					$actions['delete'] = sprintf(
						...
						$delete_ays,
						esc_attr( sprintf( 'Delete &#8220;%s&#8221; permanently', $att_title ) ),
						'Delete Permanently'
					);
				}
			}
			$actions['view'] = sprintf(
				...
				esc_attr( sprintf( 'View &#8220;%s&#8221;', $att_title ) ),
				'View'
			);
			...
			if ( current_user_can( 'edit_post', $post->ID ) ) {
				$actions['attach'] = sprintf(
					'<a href="#the-list" onclick="findPosts.open( \'media[]\', \'%s\' ); return false;" class="hide-if-no-js" aria-label="%s">%s</a>',
					$post->ID,
					esc_attr( sprintf( 'Attach &#8220;%s&#8221; to existing content', $att_title ) ),
					'Attach'
				);
			}
		}
			...
			if ( current_user_can( 'delete_post', $post->ID ) ) {
				if ( $this->is_trash ) {
					$actions['untrash'] = sprintf(
						...
						esc_attr( sprintf( 'Restore &#8220;%s&#8221; from the Trash', $att_title ) ),
						'Restore'
					);
